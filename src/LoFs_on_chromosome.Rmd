...
```{r load data}
karyotype <- read.table("../data/karyotype.hg19.txt", sep = "\t", header = T, 
                        stringsAsFactors = F, 
                        colClasses = c("character", "integer", "integer", "integer", "integer"))
head(karyotype)

mydata <- read.table("../data/cytoBandCol.hg19.txt", sep = "\t", header = T, stringsAsFactors = F, colClasses = c("character", "integer", "integer", "integer"))
head(mydata)

mydata_interval <- read.table("../data/human_knockouts_pos.hg19.txt", 
                              sep = "\t", header = T, 
                              stringsAsFactors = F,
...
```{r draw chromosome}
mpx<-3.543307
chr_width <- 170 / (2*nrow(karyotype)) * mpx
...
maxchrlen<-150
karyotype$y1 <- karyotype$y2 <- apply(data.frame(karyotype$End),1,function(x)((25+maxchrlen*(1-x[1]/max(karyotype$End))) * mpx + chr_width/2))
...
mydata$color <- colorRampPalette(c("firebrick3", "white", "black"))(cnum)[round(rescale(mydata$Value,to=c(1,cnum)))] #根据value配置颜色，使用白色则要求mydata中基因组均有value
...
mydata_interval<-rbind(mydata_interval_circle)
