...
'''
A modification version of https://github.com/OpenGene/BamQC/blob/master/bamqc.py
'''
...
def BamQC(samfile, target_area):
    ...
    return all_alignments, mapping_rate, mapping_quality, insert_size, duplication_rate
...
def main():
    ...
    all_alignments, mapping_rate, mapping_quality, insert_size, duplication_rate = BamQC(samfile, target_area)    
    capture_rate, depth_in_target, target_coverage, target_20X = BedBamQC(samfile, target_area, 20)

    f = sys.stdout if options.output == None else open(options.output, 'w')
    print("Alignments: %d" %all_alignments, file = f)
    ...
    print("Target>20X: %.3f%%" %target_20X, file = f)
